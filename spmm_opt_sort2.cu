...
__global__ void spmm_kernel_opt_sort2(const int *_block4, const int *coo_row, const int *idx, const float *val, const float *vin, float *vout, const int num_v, const int num_e, const int feat_in, const float *vout_ref)
{
...
#pragma unroll
        ...
        if (warps_per_row > 1)
        {
            atomicAdd(&vout[(block_row_begin + warp_loc_row) * feat_in + lane_id], tmp);
        }
        else
        {
            // atomicAdd(&vout[(block_row_begin + wid) * feat_in + lane_id], tmp);
            
            if (block_degree <= DEG_BOUND)
            {

                vout[(block_row_begin + wid) * feat_in + lane_id] = tmp;
            }
...
void SPMM_OPT_SORT2::run(int dim)
{
    int shared_size = (WARPS_PER_BLOCK + 0 * WARPS_PER_BLOCK / 2) * DIM_MUL(dim) * sizeof(float);
    spmm_kernel_opt_sort2<<<grid, block, shared_size>>>(_block4, 0, idx, val, vin, vout, num_v, num_e, dim, 0);
}

double SPMM_OPT_SORT2::do_test(bool timing, int dim)
{
